...
const DRIVE_FOLDER_ID = "16VCSkD551XBNVgy0q48n4PeNVKxFcwpK";// Netlify Function: /api/process
// Full workflow per livraison:
// 1. Write nb_personnes to A3 + numero_commande to F1 of source sheet
// 2. Export sheet tab as PDF
// 3. Merge with original BL PDF (BL first, then sheet)
...
exports.handler = async function(event) {
  ...
  try {
    ...
    const description = [
      ...
      `<b>Salle :</b> ${salle}`,
      contact ? `<b>Contact :</b> ${phoneRaw ? contact.replace(phoneMatch[0], `<a href="tel:${phoneRaw}">${phoneMatch[0]}</a>`) : contact}` : "",
      uploadedFile.id
        ? `<b>Document :</b> <a href="https://drive.google.com/file/d/${uploadedFile.id}">Ouvrir le PDF</a>`
        : ""
  ...
  try {
    ...
    const calResp = await fetch(
      `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(CALENDAR_ID)}/events`,
      {
        ...
        headers: { "Authorization": "Bearer " + googleToken, "Content-Type": "application/json" },
        body: JSON.stringify({
          summary: `🍽 ${client || ""} · ${livraison.type_prestation || ""} · ${nb} pers.`,
          location: [lieu, salle].filter(Boolean).join(" — "),
          description,
          start: { dateTime: startISO, timeZone: "Europe/Paris" },
          end:   { dateTime: endISO,   timeZone: "Europe/Paris" },
